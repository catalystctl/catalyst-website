...
**Stack:** TypeScript panel + Rust agent + containerd

Catalyst is built from the ground up as a modern Pterodactyl replacement. It uses TypeScript for the panel with a Rust node agent, containerd for the container runtime (the same one Kubernetes uses), and provides 200+ API route handlers with a native plugin system.

**Strengths:**
- Live console via panel gateway to Rust agent, plus files, SFTP, backups, scheduling
- One-command panel install (Docker Compose); nodes need containerd + agent
- 50+ granular RBAC permissions for precise access control
...
|---------|----------|-------------|---------|-------------|
| Panel language | TypeScript | PHP | PHP | Go |
